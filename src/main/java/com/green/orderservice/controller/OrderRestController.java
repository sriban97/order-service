...
public class OrderRestController {
    ...
    public ResponseEntity<OrderResponse> save(@RequestBody Order order) {
        ...
        completableFuture.whenCompleteAsync((responseEntity, error) -> {
            log.info("{} responseEntity {},error {}", LOG_NAME, responseEntity, error.toString());
            if (!ObjectUtils.isEmpty(error)) {
                log.error("{} error {}", LOG_NAME, error.getMessage());
            } else if (!ObjectUtils.isEmpty(responseEntity)) {
                if (responseEntity.getStatusCode() == HttpStatus.OK) {
                    atomicReference.set(responseEntity.getBody());
                    log.info("{} atomicReference {}", LOG_NAME, atomicReference);
                } else {
                    log.info("{} status code {}", LOG_NAME, responseEntity.getStatusCode());
                }
            }
        });
